...
class ReservoirSafetyController extends Controller
{
    ...
    public function downloadSafetyReport(ReservoirSafety $report)
    {
        ...
        try {
            ...
            if (!$report['finished_status'])
                return response()->json(['message' => 'Hoàn thành báo cáo trước khi tải.'], 500);
            ...
            for ($i = 0; $i < count($mainDams); $i++) {
                $mainDamBlock[] = [
                    ...
                    'NORMAL_CHECK_BOX' => $this->getCheckbox(!$report['main_dam_status']),
                    'ABNORMAL_CHECK_BOX' => $this->getCheckbox($report['main_dam_status']),
                    'DESCRIPTION' => $report['main_dam_description'],
                ];
            }
            ...
            for ($i = 0; $i < count($spillways); $i++) {
                $spillwayBlock[] = [
                    ...
                    'NORMAL_CHECK_BOX' => $this->getCheckbox(!$report['spillway_status']),
                    'ABNORMAL_CHECK_BOX' => $this->getCheckbox($report['spillway_status']),
                    'DESCRIPTION' => $report['spillway_description'],
                ];
            }
            ...
            $templateProcessor->setValue('HAS_OPERATING_SYS_CHECK_BOX_1', $this->getCheckbox($report['monitor_system_status']));
            $templateProcessor->setValue('HAS_OPERATING_SYS_CHECK_BOX_2', $this->getCheckbox(!$report['monitor_system_status']));
            $templateProcessor->setValue('OPERATING_SYS_DESCRIPTION', $report['monitor_system_description']);
            ...
            $outputFilePath = storage_path($report['name'] . '.docx');
        ...
        try {
            ...
            foreach ($postData['fileList'] as $file) {
                if ($file['name']) {
                    $filePath = storage_path('temporary-file-uploaded') . '\\' . $user->id . '_' . $file['name'];
                    if (File::exists($filePath)) {
                        ObjectActivityDocument::create(['object_activity_id' => $report->id, 'name' => $file['name'], 'description' => '']);

                        File::copy($filePath, $image_storage . '\\' . $report->id . '_' . $file['name']);
                        File::delete($filePath);
                    } else {
                        // return response()->json(['message' => 'Không tìm thấy file.'], 404);
                    }
    ...
    {
        try {
            $image = ObjectActivityDocument::find($imageId);
            $image_storage = storage_path('images/safety-report');
            $path = $image_storage . '/' . $image->object_activity_id . '_' . $image->name;
            if (file_exists($path)) {
                ...
                return response()->json(['message' => 'Yêu cầu thất bại, vui lòng thử lại sau.'], 500);
            }
    ...
    public function updateSafetyReport(Request $request, ReservoirSafety $report)
    {
        $postData = $request->json()->all();
        $user = auth()->user();
        $image_storage = storage_path('images/safety-report');
        ...
        try {
            ...
            foreach ($postData['fileList'] as $file) {
                if ($file['name'] && !isset($file['id'])) {
                    ...
                    // Không cần làm gì vì đây là những file đã tồn tại
                }
            }

            // reservoir id 
            foreach ($postData['deletedFileList'] as $file) {
                $filePath = $image_storage . '\\' . $report->id . '_' . $file['name'];
                if (File::exists($filePath) && isset($file['id'])) {
                    ObjectActivityDocument::where('id', $file['id'])->delete();
                    File::delete($filePath);
                }
            }
